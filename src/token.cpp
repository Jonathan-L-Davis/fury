...
bool is_valid_identifier( std::string str ){
    if( str.size() == 0 ) return false;//null string check
...
bool is_enclosing( std::string str ){
    if( str == "(" || str == ")" || str == "{" || str == "}" )
       return true;
    return false;
}
...
std::vector<token> tokenize( const std::string &file_name ){
    ...
    while( std::getline( file, line) ){//line by line
        for( line_depth = 0; (unsigned)line_depth < line.length();  ){//tokenize only via ws breaks
            ...
            if(tok.text.size()>0 ) {//don't push empty tokens, last token generated by a given line is a 'null' token
                retMe.push_back(tok);
            }
